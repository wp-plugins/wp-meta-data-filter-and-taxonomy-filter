        ...
        if (!empty($_POST))
        {
            ...
            if (is_object($post))
            {
                if ($post->post_type == self::$slug_woo_sort)
                {
                    ...
                    {
                        update_post_meta($post->ID, 'default_order', $_POST['default_order']);
                        update_post_meta($post->ID, 'default_order_by', $_POST['default_order_by']);
                        update_post_meta($post->ID, 'panel_type', $_POST['panel_type']);

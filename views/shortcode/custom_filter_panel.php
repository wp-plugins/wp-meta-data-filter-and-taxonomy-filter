...
<?php if(!empty($custom_filter_panel)): ?>    
    ...
    <ul class="mdf_custom_filter_panel">
        ...
        <?php if(MetaDataFilterCore::get_setting('show_panel_orderby_date')): ?>
        ...
        <?php if(MetaDataFilterCore::get_setting('show_panel_orderby_title')): ?>
            <li <?php if($order_by == 'title'): ?>class="meta_data_filter_order_<?php echo strtolower($order) ?>"<?php endif; ?>>
                <a href="<?php echo $url ?>&order_by=title&order=<?php echo($order == 'ASC' ? 'DESC' : 'ASC'); ?>"><?php echo _e('Title', 'meta-data-filter'); ?></a>
            </li>
        <?php endif; ?>

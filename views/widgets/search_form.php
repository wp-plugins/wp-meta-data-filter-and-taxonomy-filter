...
<p>
    ...
    <ul class="meta_data_filter_tax_ul" style="display: none;" id="mdf_filter_categories_addit_<?php echo $unique_id ?>">
    ...
    <ul id="meta_data_filter_tax_ul_<?php echo $unique_id ?>" class="meta_data_filter_tax_ul">
        ...
        foreach ($taxonomies_sorted as $name => $label) :
            ...
            <li>
                <?php
                $checked = "";
                if (isset($instance['taxonomies'][$name]) AND ! $counter) {
                    $checked = 'checked="checked"';
                    $counter++;
                }
                ?>
...
<p <?php if ($instance['ajax_items_recount'] == 'true'): ?>style="display:none;"<?php endif; ?>>
    ...
    <input type="hidden" id="<?php echo $widget->get_field_id('act_without_button'); ?>" name="<?php echo $widget->get_field_name('act_without_button'); ?>" value="0" />
    <label for="<?php echo $widget->get_field_id('act_without_button'); ?>"><?php _e('Auto submit form. In the premium version only!', 'meta-data-filter') ?>:</label>
</p>
...
<p <?php if ($instance['act_without_button'] == 'true'): ?>style="display:none;"<?php endif; ?>>
    ...
    ?>
    <input type="checkbox" disabled="" />
    <input type="hidden" id="<?php echo $widget->get_field_id('ajax_items_recount'); ?>" name="<?php echo $widget->get_field_name('ajax_items_recount'); ?>" value="0" />
    <label for="<?php echo $widget->get_field_id('ajax_items_recount'); ?>"><?php _e('AJAX items recount. In the premium version only!', 'meta-data-filter') ?>:</label>
</p>
...
<p <?php if ($instance['act_without_button'] == 'true'): ?>style="display:none;"<?php endif; ?>>
    ...
    ?>
    <input type="checkbox" disabled="" />
    <input type="hidden" id="<?php echo $widget->get_field_id('ajax_results'); ?>" name="<?php echo $widget->get_field_name('ajax_results'); ?>" value="0" />
    <label for="<?php echo $widget->get_field_id('ajax_results'); ?>"><?php _e('AJAX items output. In the premium version only!', 'meta-data-filter') ?>:</label>
</p>
